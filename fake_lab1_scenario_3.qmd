...
```{r}
## Get the data
fl1_sc3_url <- "https://raw.githubusercontent.com/pete-jacobsson/fake_lab/main/fake1_scenario3.csv" 
fl1_sc3 <- read_csv(url(fl1_sc3_url))
...
measurement <- fl1_sc3$measurement
instrument <- as.numeric(as.factor(fl1_sc3$instrument))
operator <- as.numeric(as.factor(fl1_sc3$operator))

n_total <- nrow(fl1_sc3)
n_instruments <- length(unique(fl1_sc3$instrument))
n_operators <- length(unique(fl1_sc3$operator))

measurement_mean <- mean(fl1_sc3$measurement)
measurement_sd <- sd(fl1_sc3$measurement)
...
Note: successfully picked up the clumsy operator. Likely error overestimate. 
