...
```{r}
## Get the data
fl1_sc2_url <- "https://raw.githubusercontent.com/pete-jacobsson/fake_lab/main/fake1_scenario2.csv" 
fl1_sc2 <- read_csv(url(fl1_sc2_url))
...
measurement <- fl1_sc2$measurement
instrument <- as.numeric(as.factor(fl1_sc2$instrument))
operator <- as.numeric(as.factor(fl1_sc2$operator))

n_total <- nrow(fl1_sc2)
n_instruments <- length(unique(fl1_sc2$instrument))
n_operators <- length(unique(fl1_sc2$operator))

measurement_mean <- mean(fl1_sc2$measurement)
measurement_sd <- sd(fl1_sc2$measurement)
...
Note: poor uncertainty increase detection, working detection of the actual issue.
